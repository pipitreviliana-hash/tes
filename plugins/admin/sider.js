export const run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         let sider1 = [], sider2 = []
         member.filter(v => groupSet.member[v]).map(v => sider1.push({
            jid: v,
            ...groupSet.member[v]
         }))
         member.filter(v => !groupSet.member[v]).map(v => sider2.push(v))
         ...
         if (args && args[0] == '-y') {
            ...
            if (arr.length == 0) return client.reply(m.chat, Utils.texted('bold', `🚩 There is no sider in this groupSet.`), m)
            for (let jid of arr) {
            ...
            if (sider2.length == 0 && lastseen.length == 0) return client.reply(m.chat, Utils.texted('bold', `🚩 There is no sider in this groupSet.`), m)
