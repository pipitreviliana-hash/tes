   ...
   async: async (m, {
      ...
      try {
         ...
         if (room && body && !prefixes.includes(body.charAt(0))) {
            ...
            if (room.words.includes(answer)) return client.reply(m.chat, Utils.texted('bold', `🚩 Kata "${answer}" sudah digunakan silahkan cari kata lain.`), m)
            if (!answer.startsWith(Utils.filter(room.answer))) {
               room.wrongs += 1
               scores.wrongAns += 1
               if (room.wrongs >= 3) {
                  ...
                  room.timer = setTimeout(() => {
                     if (room.playTimes == 1) {
                        return client.reply(m.chat, Utils.texted('bold', `🚩 Tidak ada aktifitas dalam kurun waktu 2 menit, permainan dibatalkan.`), room.chat).then(() => delete client.magz[room.id])
                     } else {
                        let teks = `乂  *S C O R E (5)*\n\n`
                        teks += people.slice(0, show).map(([user, data], i) => (i + 1) + '. @' + user.split`@` [0] + ' ' + (i == 0 ? '🥇' : i == 1 ? '??' : i == 2 ? '🥉' : '') + '\n    *( × )* : ' + data.wrongAns + '  –  *( √ )* : ' + data.correctAns + '  –  *Score* : ' + Utils.formatNumber(data.score)).join('\n')
                        teks += `\n\n`
                  ...
                  return client.reply(m.chat, `🚩 Kata harus dimulai dari awalan "${Utils.filter(room.answer)}".`, m)
               }
            }
