   ...
   async: async (m, {
      ...
      try {
         ...
         if (m.quoted && /Sisa|quizclue/i.test(m.quoted.text) && users.point >= 10000) {
            if (!(id in client.quiz) && /Sisa|quizclue/i.test(m.quoted.text)) return client.reply(m.chat, Utils.texted('bold', `🚩 Soal tersebut telah berakhir, silahkan kirim _${prefixes[0]}quiz_ untuk mendapatkan soal baru.`), m)
            if (body && !Utils.socmed(body)) {
               let json = JSON.parse(JSON.stringify(client.quiz[id][1]))
               const answer = body.toLowerCase()
               if ((client.quiz[id][3]).includes(answer)) return client.reply(m.chat, `💀 *"${answer}"* sudah terjawab!\n\nSilahkan cari jawaban lain, denda : *- ${Utils.formatNumber(reward)} Point*`, m).then(() => {
               ...
               if (!json.jawaban.includes(answer)) return client.reply(m.chat, `🚩 Jawaban mu salah!, *- ${Utils.formatNumber(reward)} Point*`, m).then(() => {
                  if (reward > users.point) {
                     users.point = 0
                  } else {
                     users.point -= reward
               ...
               for (let i = 0; i < json.jawaban.length; i++) {
                  if (client.quiz[m.chat][3].includes(json.jawaban[i])) {
                     clue += '```' + Utils.ucword(json.jawaban[i]) + '```\n'
                  } else {
                     clue += '```' + Utils.ucword(json.jawaban[i].replace(/[abcdefghijklmnopqrstuvwxyz]/g, '_')) + '```\n'
                  }
               }
               let pop = clue.split('\n')
               pop.pop()
               if (client.quiz[id][3].length == json.jawaban.length) {
                  ...
                  let teks = `乂  *Q U I Z*\n\n`
                  teks += people.map(([user, data], i) => (i + 1) + '. @' + user.split`@`[0] + ' ' + (i == 0 ? '🥇' : i == 1 ? '🥈' : i == 2 ? '🥉' : '') + '\n    *Answer* : ' + data.answer + '  –  *Score* : ' + Utils.formatNumber(data.score)).join('\n')
                  teks += `\n\n`
                  ...
                  return client.reply(m.chat, teks, m).then(() => {
                     if (client.quiz?.[id]?.[2]) clearTimeout(client.quiz[id][2])
